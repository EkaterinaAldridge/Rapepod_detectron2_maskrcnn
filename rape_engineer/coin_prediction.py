...
for filename in os.listdir("./rape_img/test/"):
    print("当前的图片地址：", "./rape_img/test/" + filename)
    if filename[-3:] != "jpg":
        continue
    current_coin_size, current_coin_area = coin_size.getcoin_size("./rape_img/test/" + filename)
    print("当前的coinsize", current_coin_size)
    ...
    for i in range(outputs["instances"].pred_masks.shape[0]):
        ...
        if length == 0:
            ...
            if area == 0:
                ...
                width = area / length
                current_width = width/current_coin_size
                current_length = length/current_coin_size
                current_area = area/current_coin_area
                ...
                if (current_area < 100):
                    continue
                v.draw_text("width: %.2fmm\nlength: %.2fmm\narea: %.2fmm^2\nratio2: %.2f\nratio1: %.2f" % (current_width, current_length, current_area,area_ratio, lw_ratio), rect[0])
